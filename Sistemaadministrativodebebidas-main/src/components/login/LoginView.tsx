...
export function LoginView({ onLogin }: LoginViewProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedUserType) {
      const user = mockUsers.find(u => u.role === selectedUserType);
      if (user) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div 
        ...
        style={{
          background: "linear-gradient(135deg, #0A74DA 0%, #D4E9FF 100%)",
        }}
      />
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 sm:p-10 animate-in fade-in zoom-in duration-500">
        ...
        <div className="flex justify-center mb-6">
          <img src={logoImage} alt="Daddy Bebidas" className="h-20 w-20 rounded-full shadow-lg" />
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <p className="text-center text-xs text-muted-foreground">
            © 2025 Daddy Bebidas - Rancagua 2771
          </p>
        </div>
      </div>
    </div>
